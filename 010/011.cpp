...
int problem11()
{
	...
	for (int i = 3; i < 23; ++i) {
		for (int j = 3; j < 23; ++j) {
			...
			for (int k = 0; k < 4; ++k) {
				temp[0] *= array[i  ][j+k]; // 右
				temp[1] *= array[i-k][j+k]; // 右下
				temp[2] *= array[i-k][j  ]; // 下
				temp[3] *= array[i-k][j-k]; // 左下
				temp[4] *= array[i  ][j-k]; // 左
				temp[5] *= array[i+k][j-k]; // 左上
				temp[6] *= array[i+k][j  ]; // 上
				temp[7] *= array[i+k][j+k]; // 右上
			}
			result = std::max(result, *std::max_element(temp.begin(), temp.end()));
		}
	}
